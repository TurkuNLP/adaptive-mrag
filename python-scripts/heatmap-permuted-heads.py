...
This file creates heatmap of topics-heads, in which each document is a strip. For similarity blue is negative and red is posetive. 
"""
...
def make_perm(vector_size, swap_counts):
    if swap_counts > vector_size // 2:
        raise ValueError("Too many swaps — would cause repetition.")

    perm = np.arange(vector_size)
    idx = np.random.permutation(vector_size)

    for k in range(swap_counts):
...
perm = make_perm(vector_size=4096, swap_counts=1600) #1, 15, 30, 50, 60
print(perm)
...
for doc in dataset[:total_docs]:  # Assuming dataset is loaded from JSONL
    ...
    title_full_embedding = title_full_embedding[:, perm]
...
print("cosine scores", cosine_scores[0][:10])
...
sns.heatmap(cosine_scores, cmap="coolwarm", yticklabels=yticklabels, center=0)
...
# Show plot
plt.savefig("fineweb_heatmap_topics_perm_random2.png", dpi=300, bbox_inches="tight")
plt.show()
plt.close()