...
This file creates a tsne projection of heads.
"""
...
def visualize_embedding_heads_with_magnitude_shading(embeddings, method='tsne', ax=None):
    ...
    scrambled_embeddings = np.empty_like(embeddings_array)
    for i in range(n_samples):
        scrambled_embeddings[i] = np.random.permutation(embeddings_array[i])
    num_heads = 32
    head_size = embedding_dim // num_heads
    print("n_samples, num_heads, embedding_dim, head_size:", n_samples, num_heads, embedding_dim, head_size)

    # Step 1: Split each embedding into 32 parts (heads)
    """num_heads = 32
    head_size = embeddings_array.shape[1] // num_heads"""
    split_embeddings = scrambled_embeddings.reshape(n_samples, num_heads, head_size)
    ...
    base_colors = [
        'red', 'firebrick', 'maroon', 'crimson',
        'orange', 'darkorange', 'gold', 'khaki',
        'green', 'limegreen', 'darkgreen', 'mediumseagreen',
        'blue', 'royalblue', 'deepskyblue', 'steelblue',
        'purple', 'indigo', 'mediumvioletred', 'orchid',
        'saddlebrown', 'chocolate', 'peru', 'tan',
        'pink', 'hotpink', 'lightpink', 'palevioletred',
        'gray', 'dimgray', 'slategray', 'lightgray',
    ]
    ...
    for head_idx in range(num_heads):
        ...
        #           color=base_colors[head_idx], label=f'Head {head_idx+1}', s=1, alpha=0.7)
        color =  base_colors[head_idx]
    ...
    ax.set_title(f'Embedding Visualization of Attention Heads ({method.upper()})') #with Magnitude Shading
    ax.set_xlabel('Component 1')
    ax.set_ylabel('Component 2')
    ax.legend(title="Heads", loc='center left', bbox_to_anchor=(1, 0.5), fontsize='small', ncol=2)
    plt.savefig("fineweb_sfr_heads_s3_scr.png", dpi=300, bbox_inches="tight")
    plt.show()
    plt.close()
...
def visualize_embedding_specific_heads_with_magnitude_shading(embeddings, specific_heads, method='tsne', ax=None):
    ...
    ax.set_title(f'Embedding Visualization of Chosen Heads ({method.upper()})') #  with Magnitude Shading
    ax.set_xlabel('Component 1')
    ax.set_ylabel('Component 2')
    ax.legend(title="Heads", loc='center left', bbox_to_anchor=(1, 0.5), fontsize='small', ncol=2)
    plt.savefig("fineweb_sfr_heads_1-4_s1.png", dpi=300, bbox_inches="tight")
    plt.show()
    plt.close()
